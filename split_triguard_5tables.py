...
OUT_DIR = Path("tri_guard_5_py_clean"); OUT_DIR.mkdir(parents=True, exist_ok=True)
...
def pick(*names): return [c for c in names if c in df.columns]

claim_keep = pick("claim_number","subrogation","claim_est_payout","liab_prct",
                  "claim_date","claim_day_of_week","channel","zip_code","zip",
...
policy_cols = pick("annual_income","high_education_ind","email_or_tel_available",
                   "address_change_ind","living_status","past_num_of_claims")
veh_cols    = pick("vehicle_made_year","vehicle_category","vehicle_price",
                   "vehicle_color","vehicle_weight","vehicle_mileage","age_of_vehicle")
...
Claim = (df[claim_cols_full]
         .drop_duplicates(subset=["claim_number"]) if "claim_number" in claim_cols_full
         else df[claim_cols_full]).copy()

def left_merge_key(base, dim, on_cols):
    return base.merge(dim, on=on_cols, how="left") if on_cols and not dim.empty else base
...
final_cols = [c for c in claim_keep if c in Claim.columns] + \
             [c for c in ["accident_key","policyholder_key","vehicle_key","driver_key"] if c in Claim.columns]
Claim = Claim[final_cols].copy()

def save_csv(df_, name): df_.to_csv(OUT_DIR / f"{name}.csv", index=False)
