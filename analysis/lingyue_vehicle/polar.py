...
# Categorical summary
cat_cols_claim = ["channel", "claim_day_of_week", "witness_present_ind", "policy_report_filed_ind"]
...
for col in cat_cols_claim:
    ...
    print(claim_clean.group_by(col).agg(pl.len().alias("count")).sort("count", descending=True))

for col in cat_cols_vehicle:
    print(f"\n{col} (Vehicle):")
    print("Unique:", vehicle_clean.select(pl.col(col).unique()).to_series().to_list())
    print(vehicle_clean.group_by(col).agg(pl.len().alias("count")).sort("count", descending=True))
...
# Regression
df = merged.select([
    "subrogation", "vehicle_price", "claim_est_payout",
    "liab_prct", "witness_present", "policy_report_filed"
]).drop_nulls()

df_pd = df.to_pandas()
y = df_pd["subrogation"]
X = df_pd[["vehicle_price", "claim_est_payout", "liab_prct", "witness_present", "policy_report_filed"]]
X = sm.add_constant(X)
